...
export default function EntregaForm() {
  ...
  useEffect(() => {
    ...
    async function loadOptions() {
      try {
        // Load turmas
        const { data: turmasData } = await getTurmas();
        if (turmasData) {
          setTurmas(turmasData);
        }

        // Load unidades
        const { data: unidadesData } = await getUnidades();
        if (unidadesData) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      const { data, error } = await registerEntrega(entregaData);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Ao selecionar sua turma, outros campos serão preenchidos automaticamente
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={field.onChange}
                      defaultValue={field.value}
                      disabled={!!form.getValues("turma_id")}
                    >
                      <FormControl>
                        <SelectTrigger>
